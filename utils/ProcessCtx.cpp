	...
	)
{
	...
	if (createInfo)
	{
		proc_info.Value = new PROC(eprocess, processId, PsGetCurrentThreadId(), createInfo);
		if (proc_info.Value)
		{
			if (m_childs.Push(proc_info))
				proc_info.Value = NULL;//avoid deleting
		}
	}
	else
	{
	...
	)
{
	...
	if (!imageInfo->SystemModeImage)
	{
		...
		img.Value = new IMG(fullImageName, processId, imageInfo);
		if (img.Value)
		{
	...
	if (!!create)
	{
		...
		if (thread_info.Value)
		{
			if (m_threads.Push(thread_info))
			{
				...
				thread_info.Value = NULL;//avoid deleting
			}
		}
	}
	else
